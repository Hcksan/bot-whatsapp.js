...
const setTemplateEcho = async function (args) {
  let message = `Hello @${args[0]} you chat is setted to ${JSON.stringify(
    args[1]
  )}\n\n${args[2]}`;
  return message;
};
...
const verifications = [
  {
    ...
    action: async (message, comand, chat, user) => {
      ...
      try {
        ...
        //==========================
        const message = await setTemplateEcho([
          user.id.user,
          myGroup,
          "Configurations has been changed",
        ]);
        //=========================
        ...
        const message = await setTemplateEcho([
          user.id.user,
          chat,
          "Configurations has been keeped",
        ]);
        //=========================
...
client.on("message", async (message) => {
  ...
  console.log(
    `Message identified on ${chat}, command identified ${comand} with label ${message.body} by contact ${user.id.name}`
  );
